...
class _PaymentsScreenState extends State<PaymentsScreen> {
  ...
  Widget _buildMonthlySummary() {
    return StreamBuilder<List<Map<String, dynamic>>>(
      stream: _firestoreService.getMonthlyPayments(_currentUserId),
    ...
    return Column(
      ...
      children: [
        ...
        StreamBuilder<List<Map<String, dynamic>>>(
          stream: _firestoreService.getUserPaymentMethods(_currentUserId),
          builder: (context, snapshot) {
            if (snapshot.connectionState == ConnectionState.waiting) {
              return const SizedBox(
                height: 180,
  ...
  Widget _buildRecentPayments() {
    return StreamBuilder<List<Map<String, dynamic>>>(
      stream: _firestoreService.getRecentPayments(_currentUserId, limit: 10),
      builder: (context, snapshot) {
        if (snapshot.connectionState == ConnectionState.waiting) {
  ...
  void _setDefaultPaymentMethod(String methodId) async {
    try {
      await _firestoreService.setDefaultPaymentMethod(_currentUserId, methodId);
  ...
  void _addPaymentMethod() async {
    ...
    try {
      ...
      await DatabaseService()
          .addPaymentMethod(widget.userId, paymentMethodData);
    ...
    return Scaffold(
      ...
      body: CustomScrollView(
        slivers: [
          ...
          StreamBuilder<List<Map<String, dynamic>>>(
            stream: DatabaseService().getPaymentHistory(userId),
            builder: (context, snapshot) {
              if (snapshot.connectionState == ConnectionState.waiting) {

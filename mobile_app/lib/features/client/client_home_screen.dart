...
class _HomeTabState extends State<HomeTab> with TickerProviderStateMixin {
  ...
  Future<List<Map<String, dynamic>>> _searchServices(String query) async {
    try {
      var servicesQuery = Supabase.instance.client
          .from('services')
          .select()
          .eq('isActive', true);

      if (_selectedCategory != 'Todos') {
        servicesQuery = servicesQuery.eq('category', _selectedCategory);
      }

      final List<dynamic> data = await servicesQuery;
      ...
      for (var item in data) {
        ...
        if (title.contains(searchTerm) ||
          ...
          final providerData = await _getProviderInfo(map['providerId']);
          map['providerInfo'] = providerData;
  ...
  Future<Map<String, dynamic>?> _getProviderInfo(String providerId) async {
    try {
      ...
      debugPrint('Error obteniendo info del proveedor: $e');
    }
    return null;
  }

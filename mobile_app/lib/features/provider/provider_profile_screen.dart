...
/// Servicio para interactuar con Firestore y Firebase Storage
class DatabaseService {
  final SupabaseClient _supabase = Supabase.instance.client;

  /// Obtiene un documento del proveedor por ID
  Stream<Map<String, dynamic>> getProvider(String providerId) {
    return _supabase
        .from('providers')
  ...
  Future<Map<String, dynamic>?> getProviderById(String providerId) async {
    try {
      ...
      logger.e('Error getting provider by ID: $e');
      return null;
    }
  }
  ...
  Stream<List<Map<String, dynamic>>> getProviderBookings(String providerId) {
    return _supabase.from('bookings').stream(primaryKey: ['id']).map((list) =>
        list.where((booking) => booking['providerId'] == providerId).toList());
  }
  ...
  Future<void> updateProvider(
    ...
    try {
      await _supabase.from('providers').update(data).eq('id', providerId);
    } catch (e) {
      logger.e('🚨 Error al actualizar proveedor: $e');
      rethrow;
    }
  }

  /// Carga una imagen a Supabase Storage y devuelve la URL
  Future<String> uploadImage(File image, String providerId) async {
    try {
      final fileName = 'profile_images/$providerId.jpg';
      ...
      logger.e('🚨 Error al cargar imagen a Storage: $e');
      rethrow;
    }
  }
}
...
class _ProviderProfileService {
  ...
  Stream<ProviderModel> getProviderProfile() {
    if (_providerId == null) {
    ...
    return _firestoreService.getProvider(_providerId!).map((data) {
      if (data.isEmpty) {
        logger.w(
      ...
      // CORREGIDO: Validación de datos antes de usar
      // data is already a Map<String, dynamic>
  ...
  Stream<List<BookingModel>> getProviderBookings() {
    ...
    return _firestoreService
        .getProviderBookings(_providerId!) // ← obligatorio usar ! aquí
        .map((bookingsList) {
      return bookingsList
          .map((data) {
            try {
              // Assuming 'id' is in the map

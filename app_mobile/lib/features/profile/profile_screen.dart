...
import '../profile/native_bridge/profile_native_bridge.dart';
...
class _ProfileScreenState extends State<ProfileScreen>
  ...
  String? _getProfileImageUrl(my_auth.AuthProvider authProvider,
    ...
    // 3. Imagen de los metadatos de Supabase Auth (ej: Google)
  ...
  Future<void> _uploadProfileImage(String imagePath) async {
    try {
      ...
      final result = await ProfileNativeBridge.uploadProfileImage(
        userId: user.uid,
        imagePath: imagePath,
      ).timeout(
        const Duration(seconds: 30),
        onTimeout: () => {'success': false, 'error': 'Timeout uploading image'},
      );

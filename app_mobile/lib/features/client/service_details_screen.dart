  ...
  Future<void> _loadServiceDetails() async {
    try {
      ...
      if (serviceData == null || serviceData!.isEmpty) {
        debugPrint('Cargando desde Supabase...');
        final serviceDoc = await Supabase.instance.client
            .from('services')
            .select()
        ...
        if (serviceDoc != null) {
          serviceData = serviceDoc;
          // serviceData!['id'] id is likely already in the map
          debugPrint(
              'Servicio cargado desde Supabase: ${serviceData!['title']}');
        } else {
          debugPrint('Servicio no encontrado en Supabase');
        }
      }

      if (serviceData != null && serviceData!['providerId'] != null) {
        debugPrint('Cargando datos del proveedor...');
        final providerDoc = await Supabase.instance.client
            .from('providers')
            .select()
            .eq('id', serviceData!['providerId'])
            .maybeSingle();

...
class _BookingScreenState extends State<BookingScreen> {
  ...
  Future<List<String>> _uploadImages() async {
    ...
    try {
      ...
      // Django endpoint for multiple file uploads or iterative single uploads
      // For now, let's assume iterative uploads to a generic media endpoint
      List<String> imageUrls = [];
      for (var file in _selectedImages) {
        final response = await apiService.postMultipart('media/upload/', file: file);
        imageUrls.add(response['url']);
      }
      return imageUrls;
    } catch (e) {
      debugPrint('Error subiendo imágenes a Django: $e');
      return [];
    } finally {
  ...
  Future<void> _confirmBooking() async {
    ...
    try {
      ...
      debugPrint('📝 Guardando reserva en Django: $bookingData');

      final apiService = BaseApiService();
      await apiService.post('bookings/', body: bookingData);
  ...
  Future<String?> _uploadReceiptImage() async {
    ...
    try {
      final apiService = BaseApiService();
      final response = await apiService.postMultipart('media/upload/', file: receiptImageFile!);
      return response['url'];
    } catch (e) {
      debugPrint('Error uploading receipt to Django: $e');
      return null;
    }
  }

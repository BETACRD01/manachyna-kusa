...
import 'provider_profile_screen.dart' hide DatabaseService;
...
class _ProviderHomeTabState extends State<ProviderHomeTab> {
      ...
      appBar: AppBar(
        ...
        actions: [
          ...
          StreamBuilder<List<Map<String, dynamic>>>(
            stream: Stream.fromFuture(BaseApiService()
                .get('users/$_uid/notifications/')
        ...
        child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
          ...
          Row(children: [
            ...
            Expanded(
              child: OutlinedButton.icon(
                onPressed: () async {
                  final recent = await DatabaseService()
                      .getProviderBookings(_uid!);
                  if (!context.mounted) return;
                      ...
                      child: ElevatedButton.icon(
                        ...
                        onPressed: () async {
                          await BaseApiService()
                              .patch('providers/$providerId/', body: {
                            'isActive': isActive,
              ...
              Expanded(
                child: ElevatedButton.icon(
                  ...
                  onPressed: () async {
                    ...
                    await BaseApiService().post('reports/', body: {
                      'userId': userId,
  ...
  Future<Map<String, dynamic>> _getStats(String providerId) async {
    final bookings = await DatabaseService().getProviderBookings(providerId);
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      ...
      body: StreamBuilder<List<Map<String, dynamic>>>(
        stream: Stream.fromFuture(DatabaseService().getProviderBookings(_uid!).then((list) => list..sort((a, b) => (b['createdAt'] ?? '').toString().compareTo((a['createdAt'] ?? '').toString()))).then((list) => list.take(100).toList())),
        builder: (context, snap) {
          if (snap.connectionState == ConnectionState.waiting) {
            return const Center(child: CircularProgressIndicator());
          }
...
class _ProviderNotificationsScreenState
  ...
  Widget build(BuildContext context) {
    ...
    final stream = Stream.fromFuture(BaseApiService().get('users/$_uid/notifications/').then((res) => List<Map<String, dynamic>>.from(res)).catchError((_) => <Map<String, dynamic>>[]));
  ...
  Future<void> _markAsUnread(String notificationId) async {
    try {
      await DatabaseService().updateDocument('notifications', notificationId, {'read': false});
    } catch (e) {
  ...
  Future<void> _markAllAsRead() async {
    _showSnackBar('Funcionalidad en desarrollo', Colors.orange);
  }
